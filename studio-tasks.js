...
function renderTask(task) {
    ...
    const modeText = task.mode === 'sheet_self' ? (task.standalonePhotography ? '图片拍摄' : '表格自助') : task.mode === 'retouch' ? '精修图片' : task.mode === 'cutout' ? '白底抠图' : task.mode === 'variant' ? '变体改色' : task.mode === 'translate_image' ? '转换语言' : task.mode === 'resize_ai' ? '尺寸修改' : task.mode === 'watermark' ? '去水印' : task.mode === 'free' ? '自由模式' : '程序模式';
    const directImageTask = ['variant', 'translate_image', 'resize_ai', 'watermark'].includes(task.mode);
    ...
    if (task.status !== 'done' && task.status !== 'rejected') {
        ...
        html += `
            <div style="background:#f8fafc;border-radius:8px;padding:12px 16px;margin-bottom:12px">
                ...
                ${task.status === 'processing' ? '<div style="font-size:0.72rem;color:#3b82f6;text-align:center;margin-top:6px">' + (task.mode === 'sheet_self' ? `已完成 ${Number(task.sheetSelfCompletedCount || 0)}/${Number(task.sheetSelfSlotCount || 0)} 张，每完成一张就会发到钉钉` : task.mode === 'retouch' ? '⏱ 图片正在精修中，预计约 30 分钟完成...' : task.mode === 'cutout' ? '⏱ 正在进行白底抠图，完成后会通过钉钉通知...' : task.mode === 'watermark' ? '⏱ AI 正在去除水印，完成后会通过钉钉通知...' : task.mode === 'translate_image' ? `⏱ 正在逐张转换语言，已完成 ${Number(task.translationNextIndex || 0)}/${Number(task.refKeys?.length || 0)} 张...` : '⏱ AI 正在生成中，预计还需 4-8 分钟...') + '</div>' : ''}
