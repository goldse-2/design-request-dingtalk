...
export async function onRequestPost({ request, env }) {
    ...
    try {
        ...
        if (action === 'extract_copy') {
            const raw = await generateAiText(env, {
                model: 'gpt-5.6-luna',
                system: '你是电商图片文字识别与翻译助手。只识别图片中真实可见的文案，严禁润色、补充或猜测；将识别到的非中文文案准确翻译成自然的简体中文，原本就是中文的内容保持不变。产品型号、专有缩写、数字、单位和参数必须保留。按视觉层级区分主标题、副标题和其他文案。只返回严格 JSON，不要代码块或解释，格式必须为 {"title":"","subtitle":"","otherText":""}。只有能够明确判断为主标题或副标题的文字才能分别填入 title 或 subtitle；无法确定是否属于标题的文字必须放入 otherText。otherText 中的每段中文文案都必须标注图片内的具体位置，格式为“左上角：中文文案”“画面底部：中文文案”等，并按从上到下、从左到右的阅读顺序使用中文分号分隔。品牌 Logo 中的文字不识别；模糊、遮挡或无法确认的文字不要猜测。所有输出内容必须让中文用户直接看懂，没有对应内容的字段返回空字符串。',
                user: '请读取这张竞品图片中的可见文案，准确转换为简体中文，并按主标题、副标题和带具体位置的其他文案分类输出。',
                images: [image],
                temperature: 0,
                maxTokens: 600,
                timeoutMs: 35000
